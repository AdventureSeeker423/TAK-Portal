...
function requireLegacyLocator(id) {
  const loc = locatorsSvc.getById(id);
  if (!loc || locatorsSvc.isLiveLocator(loc)) {
...
router.get("/locators", async (req, res) => {
  try {
    const locators = locatorsSvc.listLocatorsForAdmin({ kind: "legacy" });
...
router.patch("/locators/:id", async (req, res) => {
  try {
    ...
    const before = requireLegacyLocator(id);
...
router.post("/locators/:id/manual-ping", async (req, res) => {
  try {
    const id = String(req.params.id || "").trim();
    const loc = requireLegacyLocator(id);
    if (loc.archived) {
