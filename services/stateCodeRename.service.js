/**
 * Rename a state code for all agencies in that state and rename state TAK groups.
 */
...
async function renameStateCode(agencyIndex, newStateRaw) {
  ...
  for (let i = 0; i < agencies.length; i++) {
    const ag = agencies[i];
    if (!ag) continue;
    if (normalizeStateCode(ag.state) === oldState) {
      matchingIndexes.push(i);
    }
  }

  if (!matchingIndexes.length) {
    throw new Error("No matching agencies found for this state");
  }
